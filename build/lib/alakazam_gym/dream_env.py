...
class LocalDreamEnv(gym.Env):
    ...
    Observation (Dict) — proximity uses the {"left","right"} dict form, the
    SAME shape as the hosted sim API and the exam's policy slot, so a policy
    reading obs["proximity"]["left"] runs unchanged across all three:
      frame      Box(-1, 1, (3, 64, 64), float32) — the model's rendered view
                 (the physics exam has NO frame — do not depend on it there)
      proximity  Dict{left: Box(0,1), right: Box(0,1)} — calibrated virtual
                 range readout from the 64-ray depth fan; 0 = clear,
    ...
    def _obs_dict(self, frame, depths, blocked):
        ...
        return {
            ...
            "proximity": {"left": float(pl), "right": float(pr)},
